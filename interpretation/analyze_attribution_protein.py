...
# ============================================================
# 2. protein-specific hidden states / decoding
# ============================================================

def compute_hidden_states_before_pooling_protein(
    model,
    input_ids,
    attention_mask,
    omics_id,
    ...
    if oid != OMICS2ID["Protein"]:
        raise ValueError("This script is for Protein only.")
...
def token_to_residue_importance(token_imp: torch.Tensor, target_seq_len: int = 41) -> torch.Tensor:
    """
    Protein tokenizer is residue-level with spaces between amino acids.
    We assume residue importance should align directly to 41 positions.
    If token length differs, crop/pad conservatively.
    """
    B, L = token_imp.shape
    if L == target_seq_len:
        return token_imp
    ...
    # pad if shorter
    out = torch.zeros((B, target_seq_len), device=token_imp.device, dtype=token_imp.dtype)
    out[:, :L] = token_imp
    return out


def compute_saliency_protein(
    model,
    h: torch.Tensor,
    cond: torch.Tensor,
    ...
    residue_saliency = token_to_residue_importance(token_saliency, target_seq_len=seq_len)
    return residue_saliency, logits.detach(), probs.detach()


def compute_integrated_gradients_protein(
    model,
    h: torch.Tensor,
    cond: torch.Tensor,
    center_idx: torch.Tensor,
    ...
    residue_ig = token_to_residue_importance(token_ig, target_seq_len=seq_len)
    return residue_ig
...
def extract_attribution_protein(
    model,
    dataset,
    batch_sampler_cls,
    batch_size: int,
    ...
    for batch in loader:
        ...
        h, cond = compute_hidden_states_before_pooling_protein(
            model=model,
            input_ids=input_ids,
        ...
        saliency, logits, probs = compute_saliency_protein(
            model=model,
            h=h,
            cond=cond,
        ...
        ig = compute_integrated_gradients_protein(
            model=model,
            h=h,
            cond=cond,
            center_idx=center_idx,
...
def main():
    parser = argparse.ArgumentParser(description="Protein attribution analysis for EpiLingo")
    parser.add_argument("--framework_path", type=str, default="EpilingoHierarchicalFramework.py")
    parser.add_argument("--checkpoint_path", type=str, required=True)
    parser.add_argument("--data_root", type=str, default="./data")
    parser.add_argument("--output_dir", type=str, default="./protein_attribution_analysis")
    ...
    print(f"Selected Protein samples: {len(selected_samples)}")
    if len(selected_samples) == 0:
        raise RuntimeError("No Protein samples selected. Check filters.")
    ...
    attribution_df = extract_attribution_protein(
        model=model,
        dataset=dataset,
